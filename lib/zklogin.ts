...
export default class ZkLogin {
  ...
  private getRest = async (setLogState: (logState: LoginState) => void) => {
    ...
    const local_storage_secret_key = localStorage.getItem(LOCAL_STORAGE_SECRET_KEY);
    if (local_storage_secret_key == null) {
      throw new Error("Secret key not found");
    }
    this.maxEpoch = Number(localStorage.getItem(LOCAL_STORAGE_MAX_EPOCH));
    ...
    if (!partialZkLogin) {
      ...
      localStorage.setItem(LOCAL_STORAGE_ZKP, JSON.stringify(this.partialZkLoginSignature));
    } else {
  ...
  private prepare = async () => {
    ...
    if (local_storage_secret_key !== null) {
      ...
      localStorage.setItem(LOCAL_STORAGE_SECRET_KEY, this.ephemeralKeyPair.getSecretKey());
    }
    const { epoch } = await this.suiClient.getLatestSuiSystemState();
    this.maxEpoch = Number(epoch) + 2;
    localStorage.setItem(LOCAL_STORAGE_MAX_EPOCH, this.maxEpoch.toString());
  ...
  getURL = async () => {
    ...
    const nonce = await this.prepare();
